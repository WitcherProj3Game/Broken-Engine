...
bool ImGui::verticalText(const char* text, ImVec2* padding)
{
    ...
    ImGui::PushID(text);
    ImGui::Text("", ImVec2(text_size.y + pad * 2, text_size.x + pad * 2));
    while ((c = *text++)) {
...
//-------------------------------------------------------------------------
int ImGui::CurveEditor(const char* label
    , float* values
    , int points_count
    , float& multiplier
    ...
    , int* new_count)
{
    ...
    if (flags & (int)CurveEditorFlags::SHOW_GRID)
    {
        ...
        for (int i = 0; i < cell_cols + 1; ++i)
        {
            ...
            if (exp > 0)
            {
                ImFormatString(buf, sizeof(buf), " %.1d", int(x + i * step_x));
            }
            else
            {
                ImFormatString(buf, sizeof(buf), " %.1f", x + i * step_x);
            }
            window->DrawList->AddText(b + ImVec2(-15,(a.y - b.y)), 0x55000000, buf);
        }
        ...
        ImVec2 a0 = transform({ from_x, height });
        ImVec2 b0 = transform({ from_x + width, height });
        window->DrawList->AddLine(a0, b0, 0x55000000);

        ImGui::SetCursorPos(ImVec2(0,10));
        ImGui::SetNextItemWidth(40);
        ImGui::DragFloat("", &multiplier, 1,0, 1000,"%.1f");
        for (int i = 0; i < cell_rows; ++i)
        {
            ...
            if (exp > 0)
            {
                ImFormatString(buf, sizeof(buf), " %.1d", int(y + i * step_y * (int)multiplier));
            }
            else
            {
                ImFormatString(buf, sizeof(buf), " %.1f", y + i * step_y * (float)multiplier);
            }
            window->DrawList->AddText(a - ImVec2(40,8), 0x55000000, buf);
        }
    }
